...
//	Platform,
	...
	LoadingController,
	IonSlides,
	AlertController,IonSelect,MenuController, Platform, IonRouterOutlet, 
} from '@ionic/angular';
...
//import {IMG} from '../Services/constants.service';
...
// import { AboutUsPage } from '../app/aboutus/aboutus.page';
import { UniqueDeviceID } from '@ionic-native/unique-device-id/ngx';
import { NativeGeocoder, NativeGeocoderResult, NativeGeocoderOptions} from '@ionic-native/native-geocoder/ngx';
import { ModalController,IonContent } from '@ionic/angular';
import { DecimalPipe } from '@angular/common';
// import { MapshowPage } from '../mapshow/mapshow.page';
 ...
 // id2: any;
  ...
  // .catch((error: any) => console.log(error));
    ...
    // this.backButtonEvent();
      ...
      // console.log ('exit', this.locationCoords );
       ...
       //  this.questionList = this.questionData;
        ...
        //  this.loadQuestion(this.currentQuestion);
         // this.questionData.map((item)=>{
                //   this.another_questionList = item.categorywithqestions;
               // //  this.c = item.categorywithqestions.length;
                //  // console.log("c", this.another_questionList);
                //  item.categorywithqestions.map((item1)=>{
                //     if(item1.relatedquestions !== [])
                //     {
...
//question load 
  ...
  for(var i = 0;i <=this.tempArray2.length;i++) { 
    ...
    var ques = this.questionList[this.currentQuestion-1];
    
  for(var l = 0;l <= ques.categorywithqestions.length;l++)
   {
     var qtype = ques.categorywithqestions[l];  
        ...
        if(qtype.api_response == null)
        {
         ...
         await this.new_fun(ques.categorywithqestions[l],qtype.title,qtype.q_type,this.locationCoords.latitude,
            ...
            //   }
              ...
              // else if(qtype.id == "89" || qtype.id == "90" || qtype.id == "91")
              // {
              //    await this.present().catch(() => {}); 
              //   if(qtype.api_response != null)
              //   {
          ...
          //  await this.loadingCtrl.dismiss().catch(() => {});
            ...
            //  console.log("errorif", this.ques);
            
         }
         
        else
        {
        //  if(qtype.id == "89" || qtype.id == "90" || qtype.id == "91")
        //   {
        //     // await this.present().catch(() => {}); 
        //     if(qtype.api_response != null)
        //     {
  ...
  async new_fun(full_ques,title,q_type,latitude,longitude,currentQuestion)
{
  await this.present().catch(() => {});
   ...
   this.api.postApi('categoryquestion_googleapi', body).then(async (data: any)=> {    //all complaint listing
    const status = data.status || false;
    if(status == '200'){
      // if(full_ques.id == "89" || full_ques.id == "90" || full_ques.id == "91")
      //         {
      //            await this.present().catch(() => {}); 
      //           if(full_ques.api_response != null)
      //           {
      //             await this.dismiss().catch(() => {});
      //           }
      //           else
      //           {
      //             await this.new_fun(full_ques,title,q_type,latitude,longitude,currentQuestion);
      //           }
      //         }
     
      // if(full_ques.new_id == "19")
      // {
      //   await this.dismiss().catch(() => {});
...
previousQuestion(currentQuestion){
  ...
  //   this.questionList1 = item.categorywithqestions;
  //    this.c = Object.keys( this.questionList1).length;
  //   this.questionList1.map((item1)=>{
  //     // console.log("category" + item1);
  //      item1.answerFilled - this.c;
      
  //     // console.log("category" + item1.seq);
  //     // ++item1.seq;
  //    })
  //  })
}
    ...
    this.ques.categorywithqestions.map((item)=>{
      ...
      else if((option == 0 ) && (multiple == 0))
      {
        this.show_sign = false;
          ...
          let array1 = tempArray2.filter(val => unique.includes(val));
          ...
          {
            // console.log("not equal" );
            //we want the page to not change and we want a message popping up saying “Please make sure you answered every question on the page” and the question they missed should have something (maybe a symbol such as “ ! ” or “ * ” next to it) to show the user this is the question they missed. 
            this.filteredKeywords = tempArray2.filter((word) => !unique.includes(word));
             // console.log(this.filteredKeywords ,"word");
            // for(let i = 0;i<=this.filteredKeywords.length;i++)
            // {
            //   this.filteredKeywords_new = this.filteredKeywords[i];
            //   console.log('this.filteredKeywords_new !' ,this.filteredKeywords_new );
            // }
...
completeTest(ques_id)
{
  ...
  var multiple:any;
  this.showfooter = false;
  var another_questionList_rel:any=[];
  var another_questionList: any = [];
  //  console.log(currentQuestion ,this.questionList ,this.ques);
   if(ques_id == this.ques.id)
  {
    ...
    this.ques.categorywithqestions.map((item)=>{
      ...
      // else{
        // item.relatedquestions.map((item1)=>{
        //   another_questionList_rel = item1.id;
        //   // console.log("category" + item.id);
        //   tempArray1.push(this.another_questionList);
        //  tempArray1.push(this.another_questionList_rel);
        //   console.log("tempArray categorywithqestions " + tempArray1);
...
//   //     c.push(myItems);
//   //     this.storage.set('contacts', JSON.stringify(c));
//   //     console.log("data ",c);
//   //   });

//   console.log(id,cat,score);
  
//   // this.score = 0;
//   // this.questionList.forEach((item)=>{
    
//   //   //  this.showsubcat = _.filter(_.toArray(item.options), (item1, key) => {  
//   //   //   //  console.log("ans" + item1.seq);
//   //   //    return ( data == item1.seq); 
//   //   //    });
//   //      for(let i = 0; i < this.showsubcat.length; i++) {
//   //         //  this.row = this.showsubcat[i].score;
//   //           // this.score = this.score + this.row;
//   //           // console.log(this.score);
//   //           // console.log("item.answer" + this.score); 
//   //           this.selectedArray.push(this.showsubcat[i].score);
//   //           //this.select = this.selectedArray;
//   //          console.log("item.answer" + this.selectedArray);
//   //          }
...
//when user enter this page check current location firstly
checkGPSPermission() {
...
getLocationCoordinates() {
        ...
        var error2 = error;
        this.show = true;
        // console.log("resp8" ,  JSON.stringify(error));
        // this.askToTurnOnGPS();
        // this.requestGPSPermission();
          alert('Location access has been denied. Change your setting > this app > Allow Location access is set to be Always.');
			  //this.checkGPSPermission();
...
// selectedValue(id:any,cat:any,score:any,value:any,question_id:any,q_type:any)
//     {
//       this.showfooter = false;
//       console.log("radioSelect",id,cat,score,value,question_id,q_type);
//       // if(value == 'yes')
//       // {
//       //   let myItems = { 
//       //     "id": id,
...
//     this.showModal();
//   this.helper.pushPage('/mapshow', this.data).catch(() => {});
// }
//   // this.presentAlertConfirm();
//   // this.loadMaps('mapmap1');
// this.addTonew(myItems);
//     }
      ...
      this.storage.get('_SCORE_DATA').then((strval) => {
        ...
        //  this.tempArray2.push(data.id);
          tempArray = _.filter(strval, fv => {
            ...
            tempArray.push(data);
            this.tempArray2.push(data.id);
    //this code is for not using 'no' param pass in temarray 
      //  _.forEach(tempArray, cv => {
      //       if(cv.value == 'no')
      //       {
      //         tempArray = tempArray.filter(item => (item.value !== cv.value) && (item.question_id !== cv.question_id));
      //  // console.log(tempArray);
      //       }
      //       else{
      //         //console.log('param2 yes', tempArray);
      //       }
            
      //     }); 
          this.storage.remove('_SCORE_DATA');
          this.storage.set('_SCORE_DATA', tempArray);
            // console.log('param', tempArray);
           
        // } 
        // else {
        //   // if(data==null) return;
        //   this.storage.set('_SCORE_DATA', [data]);
        // }
      }).catch(() => {});
}  
...
isValid(data) {
 ...
 data = data.trim();
//  data=this.decimalPipe.transform(data, '1.0');
 if( data){
  // return {
  //   isValid: true,
  //   message: ''(data.length > 0 ) && (data <= 10 && data > 0) && 
  // };
...
radioGroupChange1(event,option,cat,q_type) {
        //console.log("radioGroupChange",event,option);
        this.showfooter = false;
       // console.log("radioSelect",option.id,cat,option.score,event.detail.value,option.question_id,q_type);
        // 9 undefined 2 yes 9 undefined
          let myItems = { 
       "id": option.id,
       "cat":  cat,
       "score":option.score,
...
radioGroupChange2(event)
{
        ...
        this.showradio = true;
      //  console.log("hello1",event.detail.value);
       // option.selectedItem1 = 'yes';
      // console.log("hello1", this.ques.catqueans);
}
radioGroupChange3(event,option,option_rel)
{
        this.showfooter = false;
        var number_parsed: number = +event.detail.value;
        // console.log("hello",event.detail.value,number_parsed);
        //  console.log("radioGroupChange",option,option_rel);
         let myItems;
        ...
        //  console.log("radioSelect",option.id,option.cat,option.score,value,option.id,q_type,question_id);
       ...
       if(item.id == event.detail.value)
          {
           ...
           this.addTonew(myItems);
                ...
                // var lucky = this.ques.categorywithqestions.filter(function(number) {
                 
                //     return number.id == event.detail.value;
                 
                  
                // });
                   
        // console.log("hello1",lucky);
        ...
        //  if(this.option[3] == event.detail.value)
        //  {
